...
public class InstantMessenger {
	...
	public void sendMessage(DataBaseUser user, String message, DialogFrame frame){
		...
		try {
			...
			if (dataBase.getUser(destinationName)!=null){
				temp = dataBase.getUser(destinationName);
					...
					JOptionPane.showMessageDialog(frame,
							"Такого получателя не существует", "Ошибка",
							JOptionPane.ERROR_MESSAGE);
					return;
				}
			...
			out.writeUTF("false");
			socket.close();
			frame.getTextAreaIncoming().append("Я -> " + temp.getAddres()
					+ message + "\n");
